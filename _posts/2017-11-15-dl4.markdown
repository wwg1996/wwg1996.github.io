...
```
### 2.3 合并
为了更加方便，将线性部分与激活函数的部分进行合并（LINEAR->ACTIVATION ），完成公式A<sup>[l]=g(Z<sup>[l]</sup>)=g(W<sup>[l]</sup>A<sup>[l−1]</sup>+b<sup>[l]</sup>)，其中“g”可以选择sigmoid() 或 relu()。
...
```
这里我测试了一些图片，效果其实并不是很满意，一方面是训练样本还是比较少（200多张），改善，模型也是最基本的模型，到后面下一部分会对设置超参数的一些常见方法进行进一步学习，并且学习更高级的模型比如卷积神经网络，预计会比现在的模型效果更好。